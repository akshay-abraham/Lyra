                  ...
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 lg:col-span-2">
                    ...
                    {teacherData?.subjectsTaught?.length > 0 ? (
                      <div className="mb-6 animate-fade-in-up">
                          ...
                          <Select onValueChange={setSelectedSubject} value={selectedSubject} disabled={teacherData.subjectsTaught.length <= 1}>
                              <SelectTrigger className="w-full md:w-1/2 mt-2">
                                  <SelectValue placeholder="Select a subject..." />
                              </SelectTrigger>
                              <SelectContent>
                                  {teacherData.subjectsTaught.map((s: string) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                              </SelectContent>
                          </Select>
                      </div>
                    ) : (
                        !isLoadingSettings && <p className="text-muted-foreground">You are not assigned to any subjects. Please update your profile.</p>
                    )}
                    ...
                    {isLoadingSettings ? (
                      ...
                      <>
                        ...
                        <Card className="bg-card/80 backdrop-blur-sm border-primary/20 shadow-lg animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                            ...
                            <CardContent>
                                <FormField control={form.control} name="systemPrompt" render={({ field }) => (
                                    ...
                                    <FormControl><Textarea rows={8} placeholder="e.g., You are a friendly math tutor..." {...field} className="animate-glow"/></FormControl>
                                    <FormDescription>This sets the AI's personality, role, and rules. Be explicit. Markdown is supported.</FormDescription>
                                    <FormMessage />
                                    </FormItem>
                                )} />
                            </CardContent>
                        ...
                        <Card className="bg-card/80 backdrop-blur-sm border-primary/20 shadow-lg animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                            <CardHeader>
                                <CardTitle className="font-headline text-2xl flex items-center gap-2"><Sparkles /> Answer Examples (Few-shot Prompting)</CardTitle>
                                    ...
                                    <div className="space-y-2">
                                        {fields.map((field, index) => (
                                            <div key={field.id} className="flex items-center gap-2">
                                                <FormField control={form.control} name={`exampleAnswers.${index}.value`} render={({ field }) => (
                                                <FormItem className="flex-grow">
                                                    <FormControl><Input placeholder={`Example ${index + 1}`} {...field} /></FormControl>
                                                    <FormMessage />
                                                </FormItem>
                                                )} />
                                                <Button type="button" variant="ghost" size="icon" onClick={() => remove(index)} aria-label="Remove example"><X className="h-4 w-4" /></Button>
                                            </div>
                                        ))}
                                    </div>
                                    <Button type="button" variant="outline" size="sm" onClick={() => append({ value: '' })}>Add Example</Button>
                                    <FormDescription>Show the AI what a good hint or guiding question looks like.</FormDescription>
                                </div>
                            </CardContent>
                        </Card>
                      ...
                      <CardContent>
                          <Form {...testForm}>
                              <form onSubmit={testForm.handleSubmit(onTest)} className="space-y-4">
                                  <FormField control={testForm.control} name="studentQuestion" render={({ field }) => (
                                      <FormItem>
                                          <FormLabel>Student's Question</FormLabel>
                                          <FormControl><Textarea placeholder="e.g., How do I solve for x in 2x + 5 = 15?" {...field} className="animate-glow" /></FormControl>
                                          <FormMessage />
                                      </FormItem>
                                  )} />
